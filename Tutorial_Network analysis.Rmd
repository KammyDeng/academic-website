...
---------- 目录 --------------------------------------------------------------------------------------------  
---------- 1. 实例分析与技术实现：R语言 --------------------------------------------------------------  
---------- 2. 实例操作1：Deng等人(2021)冷酷无情特质网络分析研究--------------------------------  
----------实际操作练习1：使用example1演示数据进行练习--------------------------------  
---------- 3. 实例操作2：Wang等人(2022)儿童精神病态纵向网络分析研究--------------------------  
...
```{r q1, warnings = FALSE, message=FALSE}
...
```{r q3qgroup, warnings = FALSE, message=FALSE}
...
```{r q4group1, warnings = FALSE, message=FALSE}
qgraph(cor_auto(data1), groups=ICU24groups, layout= "spring") #通过"layout = "设定spring格式
```

```{r q5group1, warnings = FALSE, message=FALSE}
...
```{r q7group3, warnings = FALSE, message=FALSE}
...
```{r q8group3, warnings = FALSE, message=FALSE}
...
```{r q9group4, warnings = FALSE, message=FALSE}
...
```{r qlabel1, warnings = FALSE, message=FALSE}
...
```{r qlabel2, warnings = FALSE, message=FALSE}
...
```{r qgroup4c, warnings = FALSE, message=FALSE}
q_cor_auto1<-qgraph(cor_auto(data1),groups=ICU24groups,minimum=0.15,cut=0.4,vsize=6,
                   legend=TRUE,borders=TRUE,layout="spring",
                   palette="colorblind",
                   legend.cex=0.25,label.cex=1,height=5,width=5,
...
```{r qgroup4j, warnings = FALSE, message=FALSE}
q_cor_auto2<-qgraph(cor_auto(data2),groups=ICU24groups,minimum=0.15,cut=0.4,vsize=6,
...
```{r centrality1, warnings = FALSE, message=FALSE}
...
```{r network, warnings = FALSE, message=FALSE}
...
```{r ci, warnings = FALSE, message=FALSE,fig.width=10,fig.height=10}
...
```{r diff1, warnings = FALSE,fig.width=15,fig.height=15}
...
```{r diff2, warnings = FALSE,fig.width=20,fig.height=20}
...
```{r diff3, warnings = FALSE, message=FALSE}
#for nodes
...
#### (3) 检验节点在中心指标的稳定性
```{r stability, warnings = FALSE, message=FALSE,fig.width=5,fig.height=5}
...
#### 3.0导入数据
```{r load3, warnings = FALSE, message=FALSE}
...
```{r longnetwork, warnings = FALSE, message=FALSE}
...
```{r layout3, warnings = FALSE, message=FALSE}
...
```{r longitudinalcentrality1, warnings = FALSE, message=FALSE}
...
```{r longitudinalcentrality2, warnings = FALSE, message=FALSE}
...
### 3.1.4 网络分析的比较检验(Network Comparison Test)
这部分主要是使用上述演示数据说明使用`NetworkComparisonTest`进行比较检验分析，选取Data1和Data2作为例子进行操作说明。

```{r NCT1, warnings = FALSE, message=FALSE}
...
```{r NCT2, warnings = FALSE, message=FALSE}
...
```{r NCT8, warnings = FALSE, message=FALSE}
